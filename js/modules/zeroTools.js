...
//            image.rotate(90);
        ...
        m.getSmallUserAvatar = function(user, width, height) {
            var root = initConfiguration.getRootLocation(),
                avatar = $('<img />').attr({
                    'src' : root + initConfiguration.imagesFolder + 'def_avatar.png'
                }),
            ...
            var willpower = user.energy.totalProgress || 50,
                paper = Raphael(avatarHolder[0], 60, 60),
                circle = paper.circle(29, 29, 20),
                uuid = Raphael.createUUID(),
                pattern = document.createElementNS("http://www.w3.org/2000/svg", "pattern"),
                image = paper.image(url,0,0,1,1);
//            image.rotate(90);
            ...
            var ps = paper.path(arc([29, 29], 25, -90, finish));
            ps.attr({stroke:'#fe4d0a',"stroke-width":2});
